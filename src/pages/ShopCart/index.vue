        ...
        >
          <li class="cart-list-con1">
            <input
              ...
              @change="isChange(cartInfo, $event)"
            />
          </li>
          ...
          <li class="cart-list-con5">
            <a
              ...
              class="mins"
              @click="handler('miuns', -1, cartInfo)"
              >-</a
            >
            <input
              autocomplete="off"
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input
          class="chooseAll"
          type="checkbox"
          :checked="isAllChecked && cartInfoList > 0"
...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      // type: 区分类型 minus为减少 , add为添加 , change为手动修改输入框
      // disNnum: 形参变 +变化量(1) -变化量(-1) input(最终的个数) 不是变化的量
      // cart: 区分产品
      switch (type) {
        case "add":
          disNum = 1;
    ...
    // 修改购物车后勾选
    async isChange(cartInfo, $event) {
      try {
        let isChecked = $event.target.checked ? "1" : 0;
        await this.$store.dispatch("isCheckCart", {
    ...
    // 改变所有勾选框
    async isChangeChecked(event) {
      try {
        let isChecked = event.target.checked ? "1" : "0";
        await this.$store.dispatch("isChangeAllChecked", isChecked);
