        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a @click="addGoodsShopCart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    // 点击需改商品参数高亮
    changeAttarValue(spuSaleAttrValue, arr) {
      arr.forEach((item) => {
        item.isChecked = 0;
      });
    ...
    // 添加商品数量
    skuNumbers(event) {
      let arrNumber = event.target.value * 1;
      if (isNaN(arrNumber) || arrNumber < 1) {
        this.skuNumber = 1;
      } else {
        this.skuNumber = parseInt(arrNumber);
      }
    },

    // 点击添加购物车
    async addGoodsShopCart() {
      try {
        await this.$store.dispatch("addShopCart", {
          skuId: this.$route.params.skuid,
          skuNum: this.skuNumber,
        });
        // 商品的详情使用会话存储
        sessionStorage.setItem('addShopCart', JSON.stringify(this.skuInfo));
        this.$router.push({
          name: "addcartsuccess",
          query: { skuNum: this.skuNumber },
        });
      } catch (error) {
        alert(error.message);
      }
